...
public class JBot {
    ...
    public static boolean update()
    {
    	...
    	// this is just some test output    	
    	if (GetGameTick() == 30)
    	{
    		DebugLog("Commandline options ("+ commandLineOptions.length +"):");
    		for(int i = 0; i < commandLineOptions.length; i++)
    		{
    			if (commandLineOptions[i].length != 2)
    				DebugLog("Commandline option "+i+" faulty!");
    			else
    				DebugLog("CommandLine option " + i + ": " + commandLineOptions[i][0] + "=" + commandLineOptions[i][1] );
    		}
    	}
    	...
    	{
    		int[] c = GetAllUnits();
    		SendChatMessage("Counting "+c.length+" Units!", CHATCHANNEL_PUBLIC);
